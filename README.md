...
<!-- DRAFT-FOR-OPERATOR-EDIT: the opening paragraph below ships as a draft. The operator's edited version is canonical; do not polish his edit back. -->
...
**The ledger that quietly duplicated itself for months.** Our ledger file was configured so that when two sessions edited it at the same time, git would merge their edits automatically instead of raising a conflict. That setting is perfect for append-only logs. It is poison for a numbered list, and here is the subtle part: when two sessions each carried an *edit to the same row* and merged, git kept **both copies**. No conflict, no warning, nothing to review. The duplicate count rose **only at merge commits — never at any edit a session actually authored** — so every session looked at its own work and saw a clean file. By the time we noticed, the file held eleven duplicated rows, and the tool that was supposed to catch duplicates had been checking something else. One configuration line had turned "two people edited the same row" from a loud conflict into a silent copy-paste machine.
...
# 2. Copy the .gitattributes stanza (it REFUSES union-merge for the ledger)
